  ...
  <div class="stack-container">
    <div class="header">
      ...
      <div class="message">
        ...
        <a target="_blank" href="https://www.baidu.com/s?ie=UTF-8&wd=php <?= $message['main']['message'] ?>">[ 百度一下
          ]</a>
        <a target="_blank" href="https://www.google.com/search?q=php <?= $message['main']['message'] ?>">[ Google ]</a>
          <?php if (isset($_ENV['EP.autoAdd'])): ?>
            <button type="button" onclick="WRITE(location.origin + location.pathname + '?t=```WRITE```','<?php echo str_replace(DS,"/",$_ENV['EP.autoAddMethodName'])?>')">[ 自动构建 ]
            </button>
          <?php endif; ?>
      </div>
    </div>
            ...
            <div id="trace_info_<?php echo $k ?>" class="trace-info">
              <div class="code-segment">
                  ...
                  foreach ($m['source'] as $s_line => $s_source) {
                      if ($m['line'] == $s_line) {
                          printf('<div class="code-line active"><span class="line">%s</span>&nbsp;&nbsp;%s</div>',
                              $s_line, $s_source);
                      } else {
                          printf('<div class="code-line"><span class="line">%s</span>&nbsp;&nbsp;%s</div>', $s_line,
                              $s_source);
                      }
                  }
                  ?>
        ...
        <div class="code-segment">
            ...
            foreach ($message['main']['source'] as $s_line => $s_source) {
                if ($message['main']['line'] == $s_line) {
                    printf('<div class="code-line active"><span class="line">%s</span>&nbsp;&nbsp;%s</div>', $s_line,
                        $s_source);
                } else {
                    printf('<div class="code-line"><span class="line">%s</span>&nbsp;&nbsp;%s</div>', $s_line,
                        $s_source);
    ...
    <div style="display: block;position: relative">
      <div class="details">
        ...
        <div class="frame active" id="frame_active"
             onclick="exception.main(null)"><?php echo $message["main"]["show_file"] ?></div>
          <?php if (!empty($message['trace'])) : ?>
            <h1>Trace</h1>
              <?php foreach ($message['trace'] as $kf => $mf) : ?>
              <div class="frame" onclick="exception.track(<?php echo $kf ?>, this, '')">
          ...
          <?php if (!empty($message['previous_trace'])) : ?>
            <h1>Previous Trace</h1>
              ...
              <div class="frame" onclick="exception.track(<?php echo $pkf ?>, this, 'previous')">
