...
class UploadImages
{
    ...
    protected $status_code = array(
        ...
        -3 => '不允许上传的类型',
        0 => '上传成功',
        1 => '上传的文件超过了配置中 max_filesize 选项限制的值',
    ...
    public function cut(
        $thumb_size = '100x100',
        $coordinate = array('x' => 0, 'y' => 0, 'w' => 0, 'h' => 0),
        $save_ori_images = true
    ) {
        $ret = $this->moveUploadFile();
        if ($ret['status'] === 0) {
            $result_file_url['ori'] = $this->getSaveFileName();
            ...
            if (!empty($thumb_size)) {
                ...
                $coordinate = array_merge(array('x' => 0, 'y' => 0, 'w' => 0, 'h' => 0), $coordinate);
            ...
            return $this->result(0, $result_file_url);
        }
    ...
    /**
     * 返回base64编码图片，满足特殊情况下不对外暴露图片真实地址
     *
     * @param string $img_file
     * @param bool $delete_img_original
     *
     * @return string
     */
    function base64image($img_file, $delete_img_original = false)
    {
        if (!is_file($img_file)) {
            $img_file = $this->save_path . DIRECTORY_SEPARATOR . $img_file;
        }
        $img_content = file_get_contents($img_file);
        $info = $this->getImageInfo($img_file, 'mime');
        if ($delete_img_original) {
            unlink($img_file);
        }
        return sprintf("data:%s;base64,%s", $info, base64_encode($img_content));
    }
     ...
     * @return string
     */
    protected function getSavePath($file_name = '')
    {
        if (!empty($this->save_path) && !is_dir($this->save_path)) {
            mkdir($this->save_path, 0755, true);
        }

        $path = rtrim($this->save_path, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR;
        if (!empty($file_name)) {
        ...
        if (empty($this->upload_file_base_info)) {
            if (isset($_FILES[$this->upload_file_name])) {
                ...
                if ($base_info['error'] !== 0) {
                    ELog::error($this->status_code[$base_info['error']]);
                } else {
    ...
    protected function getSaveFileName()
    {
        ...
        $this->ext = $base_file['ext'];
        return $this->save_name . $this->ext;
    }
    ...
    protected function makeCutThumb($images_path, $thumb_size_config, $coordinate)
    {
        ...
        foreach ($thumb_size_config as $key => $val) {
            ...
            $thumb_file_name = sprintf("%s-%sx%s{$this->ext}", $this->save_name, $this->thumb['w'], $this->thumb['h']);
            $save_path = $this->getSavePath($thumb_file_name);
            $cut->setCutSize($this->thumb['w'], $this->thumb['h']);
            $status_cut = $cut->cut($coordinate, $save_path);
            if ($status_cut) {
                $result[$key] = $thumb_file_name;
            }
        }
        return $result;
    }
    ...
    protected function makeThumb($images_path, $thumb_size_config)
    {
        ...
        foreach ($thumb_size_config as $val) {
            ...
            $result[] = $Thumb->setFile($this->getSavePath(), $thumb_file_name)->setSize($this->thumb['w'],
                $this->thumb['h'])->thumb();
        }
     ...
     * @return array|bool
     */
    protected function getImageInfo($images, $key = '')
    {
        static $cache;
        if (isset($cache[$images])) {
            if ($key) {
        ...
        if (false !== $image_info) {
            ...
            $cache[$images] = $info;
            return $this->getImageInfo($images, $key);
        } else {
            return false;
        }
    }
